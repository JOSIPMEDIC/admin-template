...
const referenceManyCall = batchParams(
  (listOfManyParams: GetManyParameters[]) => {
    const token =
    ...
    for (const resource in aggregatedParams) {
      ...
      for (const param of aggregatedParams[resource]) {
        if (param.queryFilters) {
          Object.keys(param.queryFilters).map((key) =>
            searchParams.append(key, `${param.queryFilters![key]}`),
          );
        }
      ...
      axios
        .get(`/${resource}?${searchParams.toString()}`, {
          headers: {
